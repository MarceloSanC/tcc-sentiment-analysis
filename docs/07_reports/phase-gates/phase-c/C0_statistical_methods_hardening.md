...
| MCS gold | unknown (suspeito descriptive) | `gold_mcs_results.selected_in_mcs_alpha_0_05` consumido por `gold_model_decision_final` | [`pairwise.py:_compute_mcs_from_loss_matrix`](../../../../src/domain/services/gold_builders/pairwise.py) (linhas 107-180) | `STATISTICAL_TESTS.md`, `DATA_PIPELINE_WALKTHROUGH.md` | B=300 baixo; block_len=5 hard-coded; loss=squared_error; ausencia de split_signature em saida; alimentado por loss_matrix pos top-50 | Hansen-Lunde-Nason 2011; Kunsch 1989 (block bootstrap); justificativa block_len; sensibilidade B | Unit: MCS contra pacote externo em fixture pequena; contrato: saida preserva split_signature e cohort | TODO_RESEARCH | P0 |
...
| win-rate gold | descriptive | `gold_win_rate_pairwise_results`; consumido por `gold_model_decision_final` | [`pairwise.py:382`](../../../../src/domain/services/gold_builders/pairwise.py) (`output_table = "gold_win_rate_pairwise_results"`; calculo em linhas ~410-445) | `STATISTICAL_TESTS.md`, `DATA_PIPELINE_WALKTHROUGH.md` | Sem variancia/p-value; usado como sinal de selecao em decision_final; aplicado pos top-50 | Sign test / block bootstrap para inferencia; tratamento de empates | Unit: win_rate e win_rate_ex_ties em fixture; contrato: nao usar como criterio confirmatorio em decision_final; tie_rate reportado | TODO_RESEARCH | P2 |
...
| gold_model_decision_final | unknown (depende de inputs P0) | Selecao final de modelo por (`asset`, `horizon`); consumido por plots oficiais | [`confidence.py:_build_model_decision_final`](../../../../src/domain/services/gold_builders/confidence.py) (linha 463); `requires_gold` em linhas 824-831; `output_table = "gold_model_decision_final"` (linha 822) | `ANALYTICS_STORE_ARCHITECTURE.md`, `METRICS_DEFINITIONS.md`, `PLOT_INTERPRETATION.md` | Ordenacao por `rank_rmse, rank_mae` (pontual); pinball/PICP/MPIW entram como colunas; `win_rate_ex_ties_mean` aparece sem disclaimer; herda riscos de DM/MCS/Holm/top-50; sem pre-registro do criterio | Selecao multi-objetivo com criterio pre-declarado | Contrato: nao usar como evidencia confirmatoria sem pre-registro; teste de invariancia que congele criterio pos-promocao | TODO_RESEARCH | P0 |
...
Nada se torna P1 antes que P0 esteja em pelo menos `METHOD_SPECIFIED`.
...
- Resultado consumido por `gold_model_decision_final` (criterio "DM significativo vs baseline com Holm").
...
**Uso atual no projeto**
- `gold_mcs_results.selected_in_mcs_alpha_0_05` consumido por
  `gold_model_decision_final` ("config em MCS" como criterio binario).
...
- Evidencia:
  ...
  - **split_signature** sai do groupby (verificar linhas 365-374)
...
- Saida possivelmente sem `split_signature` em coortes onde Phase B mantem essa chave.
...
- Threshold `< 0.05` usado por `gold_model_decision_final` como criterio de
  significancia ajustada.
...
- Coluna `picp` em `gold_prediction_metrics_by_run_split_horizon` e
  agregados; usada por `gold_prediction_calibration` e por
  `gold_model_decision_final`.
...
- Variantes raw/post-guardrail materializadas em
  `src/domain/services/gold_builders/descriptive.py` (linhas 398-415).
...
**Objetivo estatistico pretendido**
Selecao final de modelo por (asset, horizon) combinando criterios.
...
academico exige pre-registro do criterio.

**Perguntas metodologicas**
- A unidade estatistica esta correta? Selecao multi-input por (asset, horizon).
