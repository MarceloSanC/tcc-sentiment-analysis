...
def test_build_loss_diff_series_preserves_fact_fold_when_dim_fold_missing() -> None:
    fact = pd.DataFrame(
        [
            _row("tft", "2020-01-04", 0.50, fold="wf_1"),
            _row("baseline", "2020-01-04", 0.60, fold="wf_1"),
        ]
    )
    dim = pd.DataFrame(
        [
            {"run_id": "tft", "fold": None, "model_version": "tft", "seed": 1},
