...
A quarta contribuição é a hierarquia explícita de evidência para análise de contribuição de famílias de indicadores, documentada em \texttt{docs/04\_evaluation/EXPLAINABILITY.md}: \textit{Variable Selection Network} nativa do TFT \cite{LIMETAL2021TFT}, importância por permutação e ablação explicativa, com regra de consistência em pelo menos dois dos três métodos para sustentar afirmações de contribuição relativa. A hierarquia incorpora também a distinção formal entre contribuição preditiva condicional ao modelo e inferência causal, vedando claims causais a partir de evidência preditiva.

A quinta contribuição é a adoção de pré-registro versionado como pré-condição bloqueante para a rodada confirmatória, conforme \texttt{docs/08\_governance/EXPERIMENT\_TRACKING\_POLICY.md}. O pré-registro fixa, antes da observação dos resultados, candidato, baselines, métrica primária, bandas de calibração, regra de desempate e contrato de variante quantilica, eliminando graus de liberdade que, na ausência da declaração antecipada, induzem \textit{p-hacking} e seleção adversa de baselines \cite{PINEAU2021REPRODUCIBILITY}.
...
As contribuições empíricas reportam, sob o protocolo estatístico acima, a calibração probabilística observada por horizonte, a comparação pareada do candidato contra os baselines pré-declarados e a contribuição relativa das famílias de indicadores. Sua consolidação depende da execução confirmatória da Fase B e da análise de contribuição da Fase C, sob o pré-registro versionado, e será incorporada nesta seção após o fechamento das duas fases.
