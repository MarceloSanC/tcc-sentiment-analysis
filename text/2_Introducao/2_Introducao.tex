...
\begin{itemize}
    ...
    \item Definir e aplicar protocolo experimental reprodutível com pré-registro versionado, particionamento temporal, múltiplas sementes, alinhamento estrito por \textit{target timestamp} e comparação contra baselines explícitos pré-declarados.
    \item Avaliar calibração probabilística com métricas completas (calibração marginal por quantil, PICP intervalar, MPIW, \textit{pinball loss}, \textit{reliability diagram}) e comparar com baselines via Diebold-Mariano com correção HAC e ajuste de Holm-Bonferroni.
